...
- Всегда обновлять session ID после входа
- Ограничивать количество попыток входа (и учитывать IP/логин отдельно)
- Использовать CAPTCHA
- Внедрять MFA с обязательной валидацией на сервере
- Проверять и логировать все bypass-попытки
- Безопасно реализовать JWT: всегда проверять подпись, запрещать `alg=none`
- В OAuth всегда проверять `state`, `redirect_uri`, использовать PKCE
